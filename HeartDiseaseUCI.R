...
Heart.reg.pred <- predict(heartlogit.reg, test.df[,-14], type = "response") 
confusionMatrix(as.factor(ifelse(Heart.reg.pred > 0.5, 1, 0)), as.factor(test.df$HasDisease))

# Accuracy in prediction is 85% using Logistic Regression Model.
...
heart.ct.predict <- predict(heartpruned.ct, test.df, type='class')
confusionMatrix(heart.ct.predict, as.factor(test.df$HasDisease))

# Accuracy in prediction is 78% using Classification trees Model.
...
Heartrf.pred.test <- predict(Heartrf,test.df,type = "class")

# generate confusion matrix
confusionMatrix(Heartrf.pred.test, as.factor(test.df$HasDisease))

# Accuracy in prediction is 87% using Random forest trees Model.
...
heartBoost.pred.test <- predict(heartboost,test.df,type = "class")
# generate confusion matrix for testing data
confusionMatrix(as.factor(heartBoost.pred.test$class), as.factor(test.df$HasDisease))

# Accuracy in prediction is 85 % using Boosted trees Model.
#----------------------------------------------------------------------------------------------
#----------------------------------------------------------------------------------------------

## FINAL RECOMMENDED MODEL

# Final Recommended model is Random Forest model with an accuracy of 87%.
